...
if __name__ == '__main__':
    ...
    def get_filename_without_angle(audio_instr_fn):
        filenames_list = []
        for f in audio_instr_fn:
            idx = f.rfind('_')
            filename = f[:idx]
    ...
    for instr in ['cello', 'doublebass', 'guitar', 'sax', 'violin']:
        ...
        audio_instr_fn = [filename for filename in os.listdir(audio_instr_path) if filename.endswith('.wav')]
        audio_instr_fn = get_filename_without_angle(audio_instr_fn)
